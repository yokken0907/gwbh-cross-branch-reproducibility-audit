...
MODELS = ("IMRPhenomXPHM-SpinTaylor", "IMRPhenomXPNR", "NRSur7dq4")
...
def verify() -> dict:
    ...
    record("candidate_frame_exactly_22_unique_events", len(events) == 22 and len(set(events)) == 22, [len(events), len(set(events))], [22, 22])

    triplets = rows(OUTPUT / "SUMMARY_TRIPLETS.csv")
    record("summary_triplets_exactly_330", len(triplets) == 330, len(triplets), 330)
    record("summary_triplets_all_pass", all(row["triplet_pass"] == "True" for row in triplets), sum(row["triplet_pass"] == "True" for row in triplets), 330)
    triplet_map = {
    ...
    record("posterior_cell_frame_22x15", len(cells) == 330, len(cells), 330)
    record("posterior_count_frame_22x3x5", len(counts) == 330, len(counts), 330)
    record("all_posterior_count_gates_pass", all(row["minimum_20_pass"] == "True" for row in counts), sum(row["minimum_20_pass"] == "True" for row in counts), 330)
    ...
    record("all_22_official_identities_pass", len(identities) == 22 and all(row["identity_pass"] == "True" for row in identities), sum(row["identity_pass"] == "True" for row in identities), 22)
    record("all_22_event_hdfs_deleted", len(identities) == 22 and all(row["deleted_after_use"] == "True" for row in identities), sum(row["deleted_after_use"] == "True" for row in identities), 22)
    ...
    expected_verdict = "SUPPORTED_RELEASE_SPECIFIC_ALTTRIAD_SUMMARY_SCREENING_CALIBRATION" if mc_rho > 0 and expected_p <= 0.05 else "NOT_SUPPORTED_RELEASE_SPECIFIC_ALTTRIAD_SUMMARY_SCREENING_CALIBRATION"
    ...
    return {
        "verification_timestamp_utc": "2026-08-13T20:35:00Z",
        "verifier": "07_REPRODUCIBILITY/verify_obrc.py",
