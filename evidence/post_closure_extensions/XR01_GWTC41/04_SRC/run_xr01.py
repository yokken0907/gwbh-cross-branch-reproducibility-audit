#!/usr/bin/env python3
"""Frozen target-value-blind runner for the GWTC-4.1 XR01 transport test."""
...
RUN_ID = "GWBH_ERIA_XR01_GWTC41_COMMON5_TRANSPORT_RUN01"
CANDIDATE_COUNT = 44
CELL_COUNT = CANDIDATE_COUNT * len(PAIRS) * len(PARAMETERS)
SUMMARY_TRIPLET_COUNT = CANDIDATE_COUNT * len(MODELS) * len(PARAMETERS)
SUMMARY_SCALAR_COUNT = SUMMARY_TRIPLET_COUNT * 3
...
def monte_carlo_test(x: Any, y: Any, batch_size: int = 20_000) -> dict[str, Any]:
    ...
    return {
        ...
        "verdict": (
            ...
            if supported
            else "NOT_SUPPORTED_CROSS_RELEASE_ALTERNATE_TRIAD_SUMMARY_SCREENING_TRANSPORT"
        ),
    }
...
def verify_inputs(root: Path) -> list[dict[str, str]]:
    ...
    if summary.stat().st_size != 209500:
        raise RuntimeError("SUMMARY_SIZE_MISMATCH")
    if digest(summary, "md5") != "1e56cb4ebd8631b6f7ccc68bd89e6319":
        raise RuntimeError("SUMMARY_MD5_MISMATCH")
    if digest(summary) != "df1b28a006c21bb91d88107934eb496451382e78e2ae6371dea6455b0fda8998":
        raise RuntimeError("SUMMARY_SHA256_MISMATCH")
    semantic = load_json(root / "01_PREFLIGHT/SEMANTIC_VALIDATION_RESULT.json")
    if semantic.get("status") != "PASS_ASYMMETRIC_ERROR_MAGNITUDES":
        raise RuntimeError("SUMMARY_SEMANTICS_NOT_VALIDATED")
    if semantic.get("candidate_summary_numeric_values_accessed") != 0 or semantic.get("candidate_posterior_elements_accessed") != 0:
        raise RuntimeError("CANDIDATE_VALUE_BLINDNESS_AT_SEMANTIC_VALIDATION_INVALID")
    candidates = read_csv(root / "03_INPUT/CANDIDATE_EVENT_FREEZE.csv")
    if len(candidates) != CANDIDATE_COUNT or len({row["event"] for row in candidates}) != CANDIDATE_COUNT:
        raise RuntimeError("CANDIDATE_FRAME_NOT_EXACTLY_44_UNIQUE_EVENTS")
    expected_triad = "|".join(MODELS)
    for row in candidates:
        if row["model_triad"] != expected_triad:
            raise RuntimeError(f"MODEL_TRIAD_DRIFT:{row['event']}")
        if row["zenodo_record"] != "20275769":
            raise RuntimeError(f"ZENODO_RECORD_DRIFT:{row['event']}")
        if row["official_filename"] != f"IGWN-GWTC4p1-18965dda8_5-{row['event']}-combined_PEDataRelease.hdf5":
            raise RuntimeError(f"OFFICIAL_FILENAME_DRIFT:{row['event']}")
    return candidates
...
def evaluate_summary(root: Path, candidates: list[dict[str, str]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    identity_fields = ["gw_name", "result_samples_key", "result_file_name"]
    numeric_fields = [f"{parameter}_{part}" for parameter in PARAMETERS for part in ("median", "lower", "upper")]
    with h5py.File(root / "03_INPUT/IGWN-GWTC4p1-18965dda8_5-PESummaryTable.hdf5", "r") as hdf:
        dataset = hdf["summary_info"]
        names = set(dataset.dtype.names or ())
        masks = [f"{field}.mask" for field in numeric_fields if f"{field}.mask" in names]
        missing = [field for field in identity_fields + numeric_fields if field not in names]
        if missing:
            raise FormalStop("NO_GO_SUMMARY_SEMANTIC_COMPLETE_FRAME", "SUMMARY_FIELDS_MISSING:" + "|".join(missing))
        identities = dataset.fields(identity_fields)[:]
        index: dict[tuple[str, str], list[int]] = {}
        for position, identity in enumerate(identities):
            key = (decode(identity["gw_name"]).upper(), decode(identity["result_samples_key"]))
        ...
        for source in candidates:
            ...
            for model in MODELS:
                ...
                if decode(identity["result_file_name"]) != manifest[event]["official_filename"]:
                    failures.append(f"SUMMARY_FILE_IDENTITY:{event}:{model}")
                # Floating-point access is restricted to the 132 locked target rows.
                row = dataset.fields(numeric_fields + masks)[position]
    ...
    if len(triplet_rows) != SUMMARY_TRIPLET_COUNT or failures:
...
def close_stop(output: Path, label: str, reason: str, started: str, posterior_scalars: int, completed_events: int, errors: list[dict[str, Any]]) -> int:
    ...
    status = {
        ...
        "completed_events": completed_events, "candidate_events_required": CANDIDATE_COUNT,
    ...
    write_json(output / "VALUE_ACCESS_ATTESTATION.json", {
        "candidate_summary_scalars_read_this_run": SUMMARY_SCALAR_COUNT,
...
def run(root: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gwbh_obrc_") as temporary:
        ...
        for position, source in enumerate(candidates, start=1):
            ...
            print(f"[{position:02d}/{CANDIDATE_COUNT}] {event} DOWNLOAD_START", flush=True)
            try:
                ...
                completed_events += 1
                print(f"[{position:02d}/{CANDIDATE_COUNT}] {event} PASS", flush=True)
            except FormalStop as stop:
                ...
                persist_partial(output, identity_rows, count_rows, cell_rows, balanced_rows, errors)
    if completed_events != CANDIDATE_COUNT or len(cell_rows) != CELL_COUNT or len(count_rows) != SUMMARY_TRIPLET_COUNT:
        return close_stop(output, "NO_GO_POSTERIOR_COMPLETE_FRAME", "FINAL_FRAME_NOT_44x15", started, posterior_scalars, completed_events, errors)
    ...
    for index, event in enumerate(events):
        keep = np.arange(CANDIDATE_COUNT) != index
    ...
    decision = {
        "run_id": RUN_ID, "decision": verdict, "decision_class": "FROZEN_GWTC41_ALL44_CROSS_RELEASE_TRANSPORT",
        "primary": primary, "candidate_events": CANDIDATE_COUNT, "posterior_cells": CELL_COUNT,
        "event_replacement_or_rescue": False,
        "claim": "Cross-release conceptual transport of a summary screening calibration, within the exact frozen GWTC-4.1 frame and substituted triad only.",
    ...
    write_json(output / "VALUE_ACCESS_ATTESTATION.json", {
        "candidate_summary_scalars_read_this_run": SUMMARY_SCALAR_COUNT,
        "candidate_posterior_scalars_read_this_run": posterior_scalars,
        "candidate_events_completed": CANDIDATE_COUNT,
        "posterior_model_parameter_cells_read": SUMMARY_TRIPLET_COUNT,
        "inter_model_distance_cells_computed": CELL_COUNT,
    ...
    write_json(output / "INTERNAL_VALIDATION.json", {
        "status": "PASS",
        "all_660_w1_values_independently_replayed": True,
        "maximum_w1_absolute_replay_difference": float(max(row["independent_W1_abs_diff"] for row in cell_rows)),
        "expected_events": CANDIDATE_COUNT, "observed_events": completed_events,
        "expected_cells": CELL_COUNT, "observed_cells": len(cell_rows),
    ...
    status = {
        ...
        "candidate_events": CANDIDATE_COUNT, "posterior_cells": CELL_COUNT,
