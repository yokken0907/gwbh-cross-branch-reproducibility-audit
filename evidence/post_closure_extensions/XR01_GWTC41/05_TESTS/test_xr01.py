...
import run_xr01 as xr01  # noqa: E402


class TestXR01(unittest.TestCase):
    def test_corrected_s90_uses_asymmetric_error_widths(self):
        ...
        self.assertAlmostEqual(xr01.corrected_s90(a, b), 1.0)
    ...
    def test_ecdf_wasserstein_matches_scipy_unequal_samples(self):
        ...
        self.assertAlmostEqual(xr01.ecdf_wasserstein(a, b), wasserstein_distance(a, b), places=14)
    ...
    def test_normalized_w1_dual_implementation(self):
        ...
        normalized, raw, scale, difference = xr01.normalized_w1(a, b)
    ...
    def test_scale_floor_for_constant_equal_samples(self):
        scale = xr01.pair_scale(np.ones(30), np.ones(40))
        self.assertEqual(scale, 1e-12)

    def test_cell_seed_is_deterministic_and_keyed(self):
        first = xr01.cell_seed("GW1", "A", "B", "p")
        self.assertEqual(first, xr01.cell_seed("GW1", "A", "B", "p"))
        self.assertNotEqual(first, xr01.cell_seed("GW2", "A", "B", "p"))

    def test_balanced_values_are_deterministic(self):
        old_repetitions = xr01.BALANCED_REPETITIONS
        old_maximum = xr01.BALANCED_MAXIMUM
        try:
            xr01.BALANCED_REPETITIONS = 7
            xr01.BALANCED_MAXIMUM = 30
            a = np.arange(100, dtype=float)
            b = np.arange(80, dtype=float) + 0.5
            first, n1 = xr01.balanced_values(a, b, 123)
            second, n2 = xr01.balanced_values(a, b, 123)
            np.testing.assert_array_equal(first, second)
            self.assertEqual(n1, 30)
            self.assertEqual(n2, 30)
        finally:
            xr01.BALANCED_REPETITIONS = old_repetitions
            xr01.BALANCED_MAXIMUM = old_maximum

    def test_ranked_correlation_allows_ties(self):
        rho, rank_x, rank_y = xr01.ranked_correlation([1, 1, 2, 3], [0, 2, 1, 4])
    ...
    def test_constant_vector_is_frozen_non_support(self):
        old_permutations = xr01.PERMUTATIONS
        try:
            xr01.PERMUTATIONS = 100
            result = xr01.monte_carlo_test(np.ones(5), np.arange(5.0))
            self.assertEqual(result["verdict"], "NOT_SUPPORTED_CONSTANT_VECTOR")
        finally:
            xr01.PERMUTATIONS = old_permutations

    def test_monte_carlo_replay_is_exact_for_fixed_seed(self):
        old_permutations = xr01.PERMUTATIONS
        try:
            xr01.PERMUTATIONS = 5000
            x = np.arange(8.0)
            y = np.asarray([0.0, 1.0, 3.0, 2.0, 5.0, 4.0, 7.0, 6.0])
            first = xr01.monte_carlo_test(x, y, batch_size=777)
            second = xr01.monte_carlo_test(x, y, batch_size=777)
    ...
    def test_candidate_frame_and_protocol_shape(self):
        ...
        self.assertEqual(len({row["event"] for row in rows}), 44)
        self.assertEqual(protocol["primary_test"]["n"], 44)
        self.assertTrue(protocol["frozen_candidate_frame"]["include_all_schema_eligible_events"])
        self.assertEqual(protocol["model_contract"]["models"], list(xr01.MODELS))
        self.assertFalse(protocol["historical_parent_effect"]["project_or_branch_scientific_closure_modified"])
